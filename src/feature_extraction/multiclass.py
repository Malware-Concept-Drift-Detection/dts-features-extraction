...
def trainAndSave(experiment,name,X,y):
    ...
    with open(os.path.join(config.RESULT_DIRECTORY,experiment,name,'trueLabel_probability_list.pickle'),'wb') as wFile:
        pickle.dump(probabilityResult,wFile)
    with open(os.path.join(config.RESULT_DIRECTORY,experiment,name,'trueLabel_predictedLabel_list.pickle'),'wb') as wFile:
...
def classify(experiment,plot):
    ...
    print(f"Training and test classifiers")
    with tqdm(total=gridSize) as pbar:
        #Add 100% dataset
        trainAndSaveExperiment(f'ft_1-ff_0-st_1-sf_0',fullX,fully)
        pbar.update(1)
...
def getFeatures(experiment,path):
    ...
    for k,v in featurePartial.items():
        ...
        featurePartial[k].to_csv(os.path.join(config.RESULT_DIRECTORY,experiment,path,f'{k}_featureImportance_top100.tsv'),sep='\t')
...
def getPacking(tple):
    ...
    with open(os.path.join(config.RESULT_DIRECTORY,experiment,path,'trueLabel_predictedLabel_list.pickle'),'rb') as rFile:
        results = pickle.load(rFile)
        for result in results:
            currentPacked = result.reset_index().merge(packed['PACKER/PROTECTOR'],left_on='sample_hash',right_on='SHA256')

            pk = currentPacked[currentPacked['PACKER/PROTECTOR']==True].apply(lambda row: True if row['TrueLabel']==row['PredictedLabel'] else False,axis=1)
            pkRatio = len(pk[pk])/len(pk)
            packedRes.append(pkRatio)
            npk = currentPacked[currentPacked['PACKER/PROTECTOR']==False].apply(lambda row: True if row['TrueLabel']==row['PredictedLabel'] else False,axis=1)
...
def buildBestAndWorst(experiment):
    ...
    #Add 100% dataset
    allPredictions.append((experiment,f'ft_1-ff_0-st_1-sf_0'))
        ...
        for sampleFold in range(n_splits):
            ...
            for familyTick in familyTicks:
                for familyFold in range(n_splits):
                    allPredictions.append((experiment,f'ft_{familyTick}-ff_{familyFold}-st_{sampleTick}-sf_{sampleFold}'))
    ...
    aggregated = allResults.reset_index().merge(dynamicResults, left_on="TrueLabel", right_on="family").set_index('TrueLabel')
    aggregated = aggregated.reset_index().merge(classesFamilies, left_on="TrueLabel", right_on="family").set_index('TrueLabel')
    ...
    packed = packed.reset_index().merge(allResults['correct'],left_on='FAMILY',right_on='TrueLabel').set_index('FAMILY')
    fig, ax = plt.subplots()
    ...
    confidenceMetric1 = avclassConfidence[['final_avc2_family','av_cnt_ratio_over_labels']].groupby('final_avc2_family').mean()
    confidenceMetric1Result = pd.concat([allResults,confidenceMetric1[confidenceMetric1.index.isin(allResults.index)]],axis=1)
    pearson = round(np.corrcoef(confidenceMetric1Result['av_cnt_ratio_over_labels'].tail(40),confidenceMetric1Result['correct'].tail(40))[0,1],2)
    ...
    grouped.to_latex(os.path.join(config.RESULT_DIRECTORY,experiment,'tbl_multiclass_bestAndWorst_grouped_dynamic.tex'))
...
def buildFeatures(experiment):
    ...
    #Add 100% dataset
    feat_tbl[(80,670)] = getFeatures(experiment,f'ft_1-ff_0-st_1-sf_0')
    ...
    for sampleTick in sampleTicks:
        ...
        for sampleFold in range(n_splits):
            ...
            for familyTick in familyTicks:
                for familyFold in range(n_splits):
                    tMean[familyTick].append(getFeatures(experiment,f'ft_{familyTick}-ff_{familyFold}-st_{sampleTick}-sf_{sampleFold}'))
    ...
    for feature in config.FEAT_ALL.values():
        ...
        fig, ax = plt.subplots()
        sns.heatmap(currentHeatmap, linewidths=0.7, annot=True, fmt=".3f",square=True, annot_kws={"fontsize":12},cbar_kws={"orientation": "horizontal", "pad":0.2}, ax=ax)
...
def buildHeatmap(experiment):
    ...
    #Add 100% dataset
    heatmap.loc[80,670] = getAccuracy(experiment,f'ft_1-ff_0-st_1-sf_0')
    ...
    for sampleTick in sampleTicks:
        ...
        for sampleFold in range(n_splits):
            ...
            for familyTick in familyTicks:
                for familyFold in range(n_splits):
                    tMean[familyTick].append(getAccuracy(experiment,f'ft_{familyTick}-ff_{familyFold}-st_{sampleTick}-sf_{sampleFold}'))
    ...
    plot = sns.heatmap(heatmap, linewidths=0.7, annot=True, fmt=".3f",square=True, annot_kws={"fontsize":12},cbar_kws={"orientation": "horizontal", "pad":0.2})
